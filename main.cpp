...
double Simulation::gradient_forward(int x)
{
    return (T[x] - T[x + 1]) / dx;
}
double Simulation::gradient_backward(int x)
{
    return (T[x - 1] - T[x]) / dx;
}
    ...
    while(time < tmax) {
        ...
        if(time >= snapshot * (1 + last_out)) {
            ...
            for(int x = 0; x < num_points; x++) {
                fprintf(f, "%.9g %.9g %.9g %.9g %.9g %.9g %d\n", R * x * dx / 1e3, time / Ma, T[x], pow(R, 2)*K[x],
                        gradient_adiabat(x) / R, calculate_heat_flow() / 1e12, is_convective(x));
            }
            fprintf(f, "\n");
            fclose(f);
